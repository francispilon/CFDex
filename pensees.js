...
client.Dispatcher.on(Events.MESSAGE_CREATE, e => {
    try {
        ...
        if (message.includes("[[") && message.includes("]]")) {
            ...
            searchByName(e, name);
        } else if (message.startsWith(prefix + "roll")) {
  	    var rollTries = 10
            var rollResult = ""

            var fesPercentage = 96.5
            var guaranteed

            message = message.replace(prefix + "roll", "");
            //Get roll tries
            ...
            while (message != "") {
                ...
                if (message.indexOf(" ") != -1)
                    option = message.substr(0, message.indexOf(" "))
                else
                    option = message
                ...
                message = message.slice(option.length, message.length)
            }

            if (isNaN(rollTries)) {
                e.message.channel.sendMessage("Please type p!roll [#][%][g5 or gf] to use this command.")
...
function sendUnitReply(e, unit) {
    ...
    e.message.channel.sendMessage("", false, {
        ...
        "timestamp": new Date(),
        "description": stars,
        "url": "http://cfdex.xyz/unit/details/" + unit.ID,
        "image": {
